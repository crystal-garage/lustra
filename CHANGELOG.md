...
### Added
- Added expression `after?` and `before?` predicates for readable greater-than and less-than comparisons between SQL expressions or literal values.
...
- Aggregate subqueries now preserve ordering, so `SUM`, `MIN`, `MAX`, and `AVG` use the intended rows with limits, offsets, and `DISTINCT ON`.
...
- Transactions now default explicitly to `READ COMMITTED`, correcting the unintended Serializable default introduced by the isolation-level fix. Explicit `Serializable` and `RepeatableRead` requests still use their requested isolation levels. Serialization failures require retrying the entire transaction; Lustra does not automatically retry them.
