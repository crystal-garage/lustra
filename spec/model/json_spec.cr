...
module ModelSpec
  describe "JSON" do
    it "load and export from JSON::Any" do
      ...
      json2 = JSON.parse(%({"tags_list": ["a", "b", "c"], "flags_other_column_name": [1, 2, 3]}))

      p = Post.new(json2)
      p.tags_list.should eq ["a", "b", "c"]
      p.flags.should eq [1, 2, 3]

      # Manage hash of JSON::Any::Type, convenient for example with web frameworks:
      json_h = json2.as_h
      p = Post.new(json_h)
      p.tags_list.should eq ["a", "b", "c"]
      p.flags.should eq [1, 2, 3]

      p.to_json(emit_nulls: true).should eq(%({"id":null,"title":null,"tags_list":["a","b","c"],"flags_other_column_name":[1,2,3],"content":null,"published":null,"time_range":null,"user_id":null,"category_id":null}))
