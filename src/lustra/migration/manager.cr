...
#
class Lustra::Migration::Manager
  ...
  @migrations : Array(Lustra::Migration) = [] of Lustra::Migration
  @loaded : Bool = false
  ...
  def ensure_ready
    unless @loaded
      Lustra::SQL.execute <<-SQL
        CREATE TABLE IF NOT EXISTS __lustra_metadatas ( metatype text NOT NULL, value text NOT NULL );
        SQL

      Lustra::SQL.execute <<-SQL
        CREATE UNIQUE INDEX IF NOT EXISTS __lustra_metadatas_idx ON __lustra_metadatas (metatype, value);
        SQL
