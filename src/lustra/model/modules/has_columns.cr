...
module Lustra::Model::HasColumns
  ...
  macro __generate_columns__
    {% for db_column_name, settings in COLUMNS %}
      ...
      {% converter = Lustra::Model::Converter::CONVERTERS[settings[:converter]] %}
      {% if converter == nil %}
        {% raise "No converter found for `#{settings[:converter].id}`.\n" +
                 "The type is probably not supported natively by Lustra.\n" +
    ...
    def validate_fields_presence
      ...
      # one (polymorphic) ancestor inheriting from Lustra::Model
      {% for ancestors in @type.ancestors %}{% if ancestors < Lustra::Model %}
        super
      {% end %}{% end %}
