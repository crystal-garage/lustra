...
module Lustra::ErrorMessages
  ...
  def migration_irreversible(name = nil, operation = nil)
    ...
    build_error_message(
      ...
      [
        ...
        "Maybe you need to manually flush the migration using Postgres. `__lustra_metadatas` table store loaded " +
  ...
  def migration_not_unique(numbers)
    build_error_message(
      "The migration manage found collision on migration number. Migrations number are: #{numbers.join(", ")}",
      {
        ...
        "By default, Lustra has a `-1` migration used internally. Do not use this migration number.",
  ...
  def converter_error(from, to)
    build_error_message(
      "Lustra cannot convert from `#{from}` to #{to}.",
      {
        "Ensure your database column type matches the column declaration in Lustra",
      }
    )
  end
  ...
  def lack_of_primary_key(model_name)
    build_error_message(
      ...
      "Add `primary_key`, or mark a column with `primary: true`.",
      {
  ...
  def polymorphic_unknown_class(class_name)
    build_error_message(
      "Impossible to instantiate a new `#{class_name}` using polymorphism.",
      {
        ...
        "Ensure the name match a fully qualified, with full path, Lustra model:\n" +
  ...
  def uninitialized_db_connection(connection)
    build_error_message(
      "You're trying to access the connection #{connection} which is not initialized",
      {
        "Use `Lustra::SQL.init(#{connection}: \"postgres://XXX...\" )` on startup of your application",
