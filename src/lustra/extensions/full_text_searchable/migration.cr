class Lustra::Migration::FullTextSearchableOperation < Lustra::Migration::Operation
  module Priority
    VERY_IMPORTANT = 'A'
    IMPORTANT      = 'B'
    NORMAL         = 'C'
  ...
  private def print_concat_rules(use_new = true)
    src_fields.join(" || ") do |(field_name, field_priority)|
      ...
      "setweight(to_tsvector(#{Lustra::Expression[catalog]}, coalesce(#{use_new && "new." || ""}#{field_name}, ''))," +
        " #{Lustra::Expression[field_priority]})"
    end
  end
...
module Lustra::Migration::FullTextSearchableHelpers
  ...
  def add_full_text_searchable(table, on : Array(Tuple(String, Char)),
    ...
    migration.add_operation(
      Lustra::Migration::FullTextSearchableOperation.new(
        table,
        on,
        catalog,
        trigger_name,
